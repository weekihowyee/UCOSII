                    ...
                    CPU_CHAR  *psrch_str)
{
    ...
    while ((srch_done == DEF_NO) && (srch_ix <= srch_len)) {
        ...
        srch_cmp1     =  Str_Cmp_N(pstr_srch_ix, psrch_str, srch_str_len);
        srch_done    = (srch_cmp1 == 0) ? DEF_YES : DEF_NO;
        srch_ix++;
    }


    if (srch_cmp1 != 0) {                                        /* If srch str NOT found, rtn NULL  (see Note #2d).     */
        return ((CPU_CHAR *)0);
    }
